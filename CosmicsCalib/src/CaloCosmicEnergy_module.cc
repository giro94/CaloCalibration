...
namespace mu2e{
  ...
  void CaloCosmicEnergy::analyze(art::Event const& event){
    ...
    for(int iClu = 0; iClu < nCluster; iClu++){
      ...
      float ErrPos  [nCrystals] = {9.81f}; // Error for a uniform distribution: 34/sqrt(12) ( 34 = crystal dimension )
      int   IDs     [nCrystals] = {0};
      float path    [nCrystals] = {0.0f};
      int   whichHit[nCrystals] = {0};
      */
      ...
      if(ncry > CutNCryHit){
        ...
        auto gfit = std::make_unique<TGraphErrors>(ncry, PosX.data(), PosY.data(), ErrPos.data(), ErrPos.data());
        ...
        for(int h=0; h< ncry; h++){
          if(_diagLevel>0){
            std::cout << "using m:" << TrkSlope << " q: " << myPoly->GetParameter(0) << " x: " <<  PosX[h] << " y: "  << PosY[h]<< " ene: " << caloClusters[iClu].caloHitsPtrVector()[whichHit[h]].get()->energyDep() << std::endl;
          }
        ...
        if(Dx < 35 || chi2norm < CutChi2Norm ){
          for(int iCry=0; iCry< ncry; iCry++){
            for(int iSiPM =0; iSiPM< nSiPMs; iSiPM++){
              ...
              hSiPMfp[IDs[iCry]][iSiPM]->Fill(caloClusters[iClu].caloHitsPtrVector()[whichHit[iCry]].get()->recoCaloDigis()[iSiPM].get()->energyDep()*cryDim/path[iCry]);
            }
          }
        }// chi2 cut
